...
def process_rules(file, prefix, rdf_data, predictions_folder, kg, pca_threshold, pca_column='PCA_valid_proportion'):
    """
    ...
    Args:
        ...
        pca_column: Which PCA column to use ('PCA_valid_proportion' or 'PCA_invalid_proportion')
    ...
    required_columns = ['Body', 'Head', 'PCA_Confidence', 'Pca_Confidence', 'Pca Confidence',
                        ...
                        'PCA_valid_proportion', 'PCA_invalid_proportion']
    found_columns = [col for col in required_columns if col in rules.columns]
    if not any(col in rules.columns for col in
               ['PCA_Confidence', 'Pca_Confidence', 'Pca Confidence', 'PCA_valid_proportion',
                'PCA_invalid_proportion']):
    ...
    print(f"Using PCA column: {pca_column}")
    ...
    # Use the specified PCA column
    if pca_column not in rules.columns:
        raise ValueError(f"Specified PCA column '{pca_column}' not found in rules file")
...
def initialize(input_config):
    ...
    Returns:
        tuple: A tuple containing the following elements in order:
            ...
            - pca_column (str): Which PCA column to use for filtering.
    """
    ...
    rules = os.path.join(f'output/{kg}/', input_data['rules_file'])
    ...
    # Get PCA column with default fallback
    pca_column = input_data.get('pca_column', 'PCA_valid_proportion')
    ...
    return prefix, rules, rdf, path, predictions_folder, constraints, kg, pca_threshold, pca_column


def transform_with_pca_suffix(enriched_kg, kg, pca_column):
    """
    Wrapper function to call transform with the appropriate suffix based on PCA column used.

    Args:
        enriched_kg: The enriched knowledge graph
        kg: The knowledge graph identifier
        pca_column: Which PCA column was used ('PCA_valid_proportion' or 'PCA_invalid_proportion')
    """
    # Determine suffix based on PCA column
    if pca_column == 'PCA_valid_proportion':
        suffix = 'pca_valid'
    elif pca_column == 'PCA_invalid_proportion':
        suffix = 'pca_invalid'
    else:
        suffix = ''  # Default no suffix if neither

    # Call the original transform function with modified KG name
    transform(enriched_kg, kg, suffix)
...
if __name__ == '__main__':
    try:
        ...
        input_config = 'input-symbolicPred.json'
        ...
        prefix, rulesfile, rdf_data, path, predictions_folder, constraints, kg, pca_threshold, pca_column = initialize(
            input_config)
        ...
        # print("\nValidating results...")
        # val_results = travshacl(rdf_data, constraints, kg)

        # Normalizing enriched KG (enrichedKG obtained from symbolic predictions) with PCA suffix
        print("\nTransforming results...")
        transform_with_pca_suffix(enriched_kg, kg, pca_column)
